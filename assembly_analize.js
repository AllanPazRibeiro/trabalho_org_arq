...
const ParseArquivo = () => {
	...
	for (line in lines) {
		...
		let variables_next = lines[parseInt(line) + 1] ? lines[parseInt(line)  + 1].match(/((\$[TS])\S?\d,\$[TS]\d,)\S((\$[TS]\d)|(\w*)\S)?/g) : '';
		let inst2 = lines[parseInt(line)  + 1] ? lines[parseInt(line) + 1].match(/ADDI|ADD|SUB|LW|J|BNE/g) : '';
		let par = lines[line].match(/\$([TS])\S/g);
		let par_next = lines[parseInt(line)  + 1] ? lines[parseInt(line) + 1].match(/\$([TS])\S/g) : '';
		...
		inst_next_arr.push(inst2);
		variables_next_arr.push(variables_next);
		par_next_arr.push(par_next);

	}		
		console.log('-------------intersection---------------------------------------------');
		intersection(variables_arr, inst_arr, par_arr, variables_next_arr, inst_next_arr, par_next_arr);
		console.log('----------------------------------------------------------------------');
		console.log('\n');
...
//Verifica a intersecção
var intersection = (vari, inst, par, vari_next, inst_next, par_next) => {
	...
	vari.map( value => aux.push(value));
	inst.map( value => aux2.push(value));
	par.map( value => aux5.push(value));

	vari_next.map( value => aux3.push(value));
	inst_next.map( value => aux4.push(value));
	par_next.map( value => aux6.push(value));
	...
	for(i in (aux5.filter((value) => aux6.includes(value) + "\n"))) {
		//if (aux5.filter((value) => aux6.includes(value) + "\n")) {
			console.log(aux2[i] + " tem conflito com " + aux4[i]);
		//}
	} 
